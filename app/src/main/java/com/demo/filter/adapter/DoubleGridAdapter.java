...
import com.demo.filter.typeview.grid_holder.ItemViewHolder;
        ...
        switch (itemViewType) {
            ...
            case TYPE_ITEM:
                ...
                if (position < topGridData.size() + 1) {
                    itemViewHolder.bind(topGridData.get(position - 1), topGridData.get(position - 1));
                } else {
                    itemViewHolder.bind(bottomGridData.get(position - topGridData.size() - 2), bottomGridData.get(position - topGridData.size() - 2));
                }
                break;
        }
    }
